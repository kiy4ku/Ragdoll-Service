...
function module:Start()
	...
	if constants.RAGDOLL_CAMERA_SPRING then
		...
		if self.torso and self.head then
			...
			self.runTimeTrove:BindToRenderStep('RagdollCameraSpring', Enum.RenderPriority.Camera.Value - 1, function()
				...
				self.cameraSpring:impulse(camera.CFrame:vectorToObjectSpace(camera.CFrame.lookVector:Cross(relativeVelocity)) / 12.5)
	...
	self.runTimeTrove:Connect(self.Instance.StateChanged, function(_: Enum.HumanoidStateType?, new: Enum.HumanoidStateType)
